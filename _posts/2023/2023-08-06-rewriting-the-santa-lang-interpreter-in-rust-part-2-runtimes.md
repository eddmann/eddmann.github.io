...
meta: 'This series of articles documents my experience rewriting the santa-lang interpreter in Rust; in this article I delve into how I went about integrating the core language library with the desired (delivery) runtimes'
...
The TypeScript implementation of the santa-lang interpreter included a CLI, Web and Lambda runtimes.
...
There are many useful [crates](https://github.com/clap-rs/clap) available to help assist in CLI development, but I did not feel like I needed the additional features they provided at this time.
...
To help aid in performance benchmarking I added the ability to compile the runtime with [pprof](https://github.com/tikv/pprof-rs) code profiling (via a [`profile`](https://github.com/eddmann/santa-lang-rs/blob/870e364a9ae47c3648e358d89b42e491f9d59577/runtime/cli/Cargo.toml#L26) feature flag).
...
The TypeScript interpreter being transpiled into JavaScript made it already well suited for _Web platform_ usage, with very little additional effort on my part.
To provide use of this interpreter within the [Web-based code editor](https://eddmann.com/santa-lang-editor/) and Browsers/Node in-general I was required to leverage WebAssembly (WASM).
Having had [experience](https://eddmann.com/posts/building-a-rubik-cube-solver-using-rust-wasm-threejs-and-react/) doing this before I was again surprised at how easy it was to achieve.
Not only was I able to trivially expose the language as a WASM library, but additionally provide the ability to include user-land JavaScript functions as external functions (similar to the TypeScript implementation), as shown below:
...
Similar to the CLI, I was able to garner confidence of the desired behaviour by using a [testing](https://crates.io/crates/wasm-bindgen-test) crate which allowed me to verify correctness of the compiled library within both Node and Chrome-browser environments.

With very [little changes](https://github.com/eddmann/santa-lang-editor/blob/b27aeddc19444003cb5d39704d934e733e05bb86/worker.ts) I was then able to update the [Web-based code editor](https://eddmann.com/santa-lang-editor/) to use this new interpreter.
...
I decided to additionally add [executable examples](https://github.com/eddmann/santa-lang/blob/f55c6d085118bc9ae6a269e2f57ef415b57b96eb/runner/runner.js) using this library, which exercised use of the runtime in a different context.
...
I used the lightweight [ureq](https://crates.io/crates/ureq/0.9.0) crate to handle HTTP transport between the compiled _bootstrap_ and Lambda runtime environment.
Due to Lambda's heavy reliance on JSON for transport, I again enabled the `serde` feature support (similar to the WASM runtime) that had been added to the core language library to handle converting santa-lang data-structures to and from their JSON equivalent.
I was pleasantly surprised how performant this runtime was in practise, with very little overheads and a small binary size.
...
I had limited experience in writing PHP extensions in the past and really enjoyed how this crate abstracted away any need of a C-macros and `zend_` magic.
...
This certainly isn't the most useful runtime, but it does document how easy it is to expose Rust-based code in a higher-level language, without the need to resort to C.
...
This is in most part due to every AI/LLM YouTube video/or article I see using it (due to the authors coming from Data Science backgrounds).
...
Now that all the desired runtimes had been implemented it was finally time to investigate performance considerations!
